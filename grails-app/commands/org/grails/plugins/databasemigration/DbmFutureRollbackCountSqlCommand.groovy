...
@Commons
@CompileStatic
class DbmFutureRollbackCountSqlCommand implements ApplicationCommand, DatabaseMigrationCommand {
    ...
    boolean handle(ExecutionContext executionContext) {
        ...
        if (!number) {
            log.error "The $name command requires a change set number argument"
            return false
        }
        if (!number.isNumber()) {
            log.error "The change set number argument '$number' isn't a number"
            return false
        }
