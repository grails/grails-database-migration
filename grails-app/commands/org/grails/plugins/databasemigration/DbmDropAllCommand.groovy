...
@CompileStatic
class DbmDropAllCommand implements ApplicationCommand, DatabaseMigrationCommand {

    final String description = 'Drop all database objects owned by user'

    @Override
    boolean handle(ExecutionContext executionContext) {
        def commandLine = executionContext.commandLine

        def schemaNames = commandLine.remainingArgs[0]
