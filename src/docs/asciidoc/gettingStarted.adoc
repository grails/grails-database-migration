...
NOTE: If you use the searchable plugin you need to configure it to run after the database migrations have run. Either in `Searchable.groovy` or in `application.groovy` (if that's where you configure Searchable):
[source,groovy]
----
