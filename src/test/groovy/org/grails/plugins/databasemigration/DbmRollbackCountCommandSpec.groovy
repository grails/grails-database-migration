...
class DbmRollbackCountCommandSpec extends DatabaseMigrationCommandSpec {

    final Class<ApplicationCommand> commandClass = DbmRollbackCountCommand
    ...
    def "an error occurs if the count parameter is not specified"() {
        ...
        then:
            !result
            outputCapture.toString().contains("The ${command.name} command requires a change set number argument")
    }

    def "an error occurs if the count parameter is not number"() {
        when:
            def result = command.handle(getExecutionContext('one'))

        then:
            !result
            outputCapture.toString().contains('The change set number argument \'one\' isn\'t a number')
    }
