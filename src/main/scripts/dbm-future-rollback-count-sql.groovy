...
import org.grails.plugins.databasemigration.command.DbmFutureRollbackCountSqlCommand

description('Writes SQL to roll back the database to the current state after <number> changes in the changeslog have been applied') {
    usage 'grails [environment] dbm-future-rollback-count-sql [number] [filename] --contexts=[contexts] --defaultSchema=[defaultSchema] --dataSource=[dataSource]'
...
try {
    new DbmFutureRollbackCountSqlCommand().handle(executionContext)
