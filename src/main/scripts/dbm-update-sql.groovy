...
import org.grails.plugins.databasemigration.command.DbmUpdateSqlCommand

description('Writes the SQL that will update the database to the current version to STDOUT or a file') {
    usage 'grails [environment] dbm-update-sql [filename] --contexts=[contexts] --defaultSchema=[defaultSchema] --dataSource=[dataSource]'
...
try {
    new DbmUpdateSqlCommand().handle(executionContext)
