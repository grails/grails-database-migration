...
description('Rolls back the database to the state it was in when the tag was applied') {
    usage 'grails [environment] dbm-rollback [tagName] --contexts=[contexts] --defaultSchema=[defaultSchema] --dataSource=[dataSource]'
...
try {
    new DbmRollbackCommand().handle(executionContext)
